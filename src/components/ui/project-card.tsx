  ...
  return (
    <article className="group relative bg-background border border-border/50 rounded-lg p-6 hover:border-border transition-all duration-300 hover:shadow-sm">
      ...
      <header className="flex items-start justify-between mb-4">
        ...
        {/* Index */}
        <div className="w-8 h-8 bg-muted/50 text-muted rounded-md flex items-center justify-center text-xs font-mono opacity-90">
      ...
      {/* Tech Stack */}
      <div className="mb-6">
        <h4 className="text-medium font-medium text-primary mb-3 ">
          Stack
        </h4>
        <div className="flex flex-wrap gap-1.5">
          {tech.map((item, techIndex) => (
            <span
              key={techIndex}
              className="text-xs bg-muted/50 border-border  hover:bg-accent px-2 py-1.5 rounded-lg"            >
              {item}
            </span>
          ))}j
        </div>
